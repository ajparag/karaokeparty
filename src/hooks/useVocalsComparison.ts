...
export function useVocalsComparison(options: UseVocalsComparisonOptions = {}) {
  ...
  const initRefFromUrl = useCallback(async (vocalsUrl: string) => {
    ...
    try {
      ...
      console.log('[vocals-comparison] Ref audio ready, ctx state:', ctx.state);
    } catch (e) {
  ...
  const startAnalysis = useCallback(async () => {
    ...
    try {
      ...
      // If ref audio was already initialised with a temporary context
      // (vocalsUrl arrived before startAnalysis), reconnect it to the
      // singleton now that it's guaranteed running via user gesture.
      if (refAudioElRef.current && refAudioCtxRef.current !== userAudioCtxRef.current) {
        // Reset and re-init so it picks up the singleton ctx
