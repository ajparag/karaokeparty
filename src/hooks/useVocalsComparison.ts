...
export function useVocalsComparison(options: UseVocalsComparisonOptions = {}) {
  ...
  // ─── setRefVolume: no-op — hook audio is muted for analysis only ─────────
  // Audible vocals volume is controlled by Sing.tsx's separate vocalsAudioRef.
  // This stays as a no-op to keep the API compatible with existing Sing.tsx calls.
  ...
  const initRefFromUrl = useCallback(async (vocalsUrl: string) => {
    ...
    try {
      ...
      // MUTED — analysis only. This element must never reach speakers.
      // If it plays audibly, the mic picks up the reference vocals, making
      // the score rise when NOT singing and fall when singing (inverted scores).
      audio.volume = 0;
      audio.muted = true;
      ...
      // The analyser reads the full signal for scoring.
      // keepAlive is near-zero so nothing audible reaches speakers.
      // Audible guide vocals are played by Sing.tsx's separate vocalsAudioRef.
        ...
        if (!didFallbackRef.current) {
          ...
          if (lowSignalFramesRef.current > 120) {
            ...
            navigator.mediaDevices.getUserMedia({
              // Enable AEC in fallback — same as primary to prevent speaker bleed
