...
export function useVocalsComparison(options: UseVocalsComparisonOptions = {}) {
  ...
  const startAnalysis = useCallback(async () => {
    ...
    try {
      ...
      } else if (url && refInitialisedUrlRef.current && refAudioElRef.current) {
        // Ref audio already initialised and element still exists (stop/start cycle).
        // Just play it — the Web Audio graph is still connected.
        console.log('[HOOK] startAnalysis: ref audio already set up, calling play()');
        if (optionsRef.current.isPlaying) {
          refAudioElRef.current.currentTime = optionsRef.current.currentTime ?? 0;
          refAudioElRef.current.play()
            .then(() => console.log('[HOOK] startAnalysis: ref audio play() ok'))
            .catch(e => console.warn('[HOOK] startAnalysis: ref play() failed', e));
        }
      } else if (url && refInitialisedUrlRef.current && !refAudioElRef.current) {
        // Ref was initialised before but element was destroyed — safe to re-init
        // because the old element is gone so no double-createMediaElementSource.
        console.log('[HOOK] startAnalysis: ref element gone, re-initialising');
        refInitialisedUrlRef.current = null;
        await initRefFromUrl(url);
      } else {
        console.warn('[HOOK] startAnalysis: no vocalsUrl yet — will init when URL arrives');
      }
      if (refAudioCtxRef.current && refAudioCtxRef.current !== userAudioCtxRef.current) {
        console.log('[HOOK] startAnalysis: syncing refAudioCtx to userAudioCtx');
